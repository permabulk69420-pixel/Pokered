...
const SPECIES=Object.freeze({
  charmander:Object.freeze({
    ...
    // Most generated character GLBs face +Z. Keep this isolated in the species
    // definition so a differently-authored model is a one-number adjustment.
    forwardYaw:0,
  }),
});

// The current Oak table is authored red / blue / leaf from left to right.
// Only Charmander has an asset today; the slot map is already ready for the
// other two species without baking Charmander logic into the lab geometry.
...
function makeAnimator(model,clips,species){
  ...
  const selected={
    idle:chooseClip(clips,species.idle),walk:chooseClip(clips,species.walk),
    run:chooseClip(clips,species.run),scratch:chooseClip(clips,species.scratch),
  };
...
function normalizeModel(model,height){
  ...
  model.updateMatrixWorld(true);bounds=new THREE.Box3().setFromObject(model);
...
export async function createPokemonSystem({renderer,scene,rig,states,onClaimBall}){
  ...
  const handWorld=new THREE.Vector3(),slotWorld=new THREE.Vector3(),ballWorld=new THREE.Vector3();
  ...
  let activeKey=null,model=null,animator=null,starterMount=null,lastSpaceVisible=false;
  ...
  async function loadSpecies(key){
    ...
    const animation=makeAnimator(root,gltf.animations,species);
    return {species,root,animation};
  }

  // Preload the one available starter so the first throw is instant on Quest.
  let charmanderAsset=null;
  try{charmanderAsset=await loadSpecies('charmander');}
  catch(error){console.warn('Charmander asset could not be loaded.',error);}

  function stateSnapshot(){return {starter:state.starter,party:state.party.map(p=>({...p})),flags:{...state.flags},active:activeKey};}
  function emit(type,detail={}){
    try{window.dispatchEvent(new CustomEvent('kanto:pokemon',{detail:{type,...detail,state:stateSnapshot()}}));}catch{}
  }
  ...
  function claimStarter(key,inputState,mount){
    ...
    starterMount=mount;mount.visible=false;
    onClaimBall?.(key,inputState);
    emit('starter-chosen',{pokemon:{species:key,id:species.id,name:species.name,level:5}});
    return true;
  }

  function checkStarterPickup(){
    if(state.flags.starterChosen||!state.flags.starterSelectionUnlocked)return;
    ...
    slot.mount.getWorldPosition(slotWorld);
    for(const inputState of states){
      const squeeze=inputState.inputSource?.gamepad?.buttons?.[1]?.value??0;
      if(!inputState.inputSource||inputState.inputSource.hand||squeeze<STARTER_GRAB_ON)continue;
  ...
  function ensureCompanion(){
    if(model||!charmanderAsset)return;
    model=charmanderAsset.root;animator=charmanderAsset.animation;companion.add(model);
  }

  function release(key,position){
    if(!state.party.some(p=>p.species===key))return false;
    if(activeKey===key)return false;
    if(key!=='charmander'||!charmanderAsset)return false;
    ...
    companion.rotation.set(0,SPECIES[key].forwardYaw,0);animator?.play('idle');
    emit('released',{species:key});return true;
  }

  function recall(key){
    if(activeKey!==key)return false;
    activeKey=null;companion.visible=false;animator?.play('idle');emit('recalled',{species:key});return true;
  }
  ...
  function ballOpened(key,position){
    ...
    companion.getWorldPosition(temp);
    if(temp.distanceTo(position)<=RECALL_RADIUS)recall(key);
  }
  ...
  function updateFollower(dt){
    ...
    // Trail just behind and a little to the player's left so the companion
    // stays visible without living directly under either hand/holster.
    desired.copy(head).addScaledVector(forward,-1.12);
    desired.x+=forward.z*.42;desired.z-=forward.x*.42;desired.y=rig.position.y;
    delta.copy(desired).sub(companion.position);delta.y=0;
    const distance=delta.length();
    if(distance>FOLLOW_SNAP){companion.position.copy(desired);animator?.play('idle');}
    else if(distance>FOLLOW_STOP){
      const speed=distance>3?FOLLOW_RUN:FOLLOW_WALK,step=Math.min(distance-FOLLOW_STOP,speed*dt);
    ...
    companion.position.y=THREE.MathUtils.lerp(companion.position.y,rig.position.y,Math.min(1,dt*8));
    animator?.update(dt);
  }
  ...
  function update(dt){
    ...
    checkStarterPickup();
    updateFollower(dt);
    if(!xr&&companion.visible)companion.visible=false;
    const lab=scene.getObjectByName('interior-oaks-lab');
    const visible=!!lab?.visible;if(visible!==lastSpaceVisible){lastSpaceVisible=visible;if(activeKey&&xr&&companion.position.distanceTo(rig.position)>FOLLOW_SNAP)companion.position.copy(rig.position);}
  }

  function resetSession(){
    if(activeKey){companion.visible=false;companion.position.copy(rig.position);}
  }

  function setStarterSelectionUnlocked(value){
    state.flags.starterSelectionUnlocked=!!value;emit('starter-lock',{unlocked:state.flags.starterSelectionUnlocked});
  }

  return {
    update,ballOpened,release,recall,resetSession,setStarterSelectionUnlocked,
    get starter(){return state.starter;},
    get party(){return state.party.map(p=>({...p}));},
    get active(){return activeKey;},
    get animations(){return animator?.names||charmanderAsset?.animation?.names||[];},
